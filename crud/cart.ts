import { Product, ProductCartItem, ServiceCartItem, ServiceCart, PrismaClient, ServiceCartStatus } from "@prisma/client";
...
export const addServiceCartItem = async (input: CreateServiceCartItemDTO, prisma: PrismaClient) => {
...
export const removeServiceCartItem = async (input: RemoveServiceCartItem, prisma: PrismaClient) => {
...
async function update(cartItemId: string, cartItem: ProductCartItemDTO, prisma: PrismaClient) {
    const cartItems = prisma.productCartItem;
...
export async function getServiceCartItem(cartItemId: string, prisma: PrismaClient) {
    const cartItems = prisma.serviceCartItem;
    const existingcartItem = await cartItems.findUnique({ where: { id: cartItemId } })
...
export async function updateServiceCartItem(cartItem: UpdateServiceCartItemDTO, prisma: PrismaClient) {
    const cartItems = prisma.serviceCartItem;
    if (cartItem.addons.length > 0) {
        ...
        return await removeServiceCartItem({ cartItemId: cartItem.cartItemId }, prisma)

    }

}

export async function updateServiceCartStatus(cartId: string, status: ServiceCartStatus, prisma: PrismaClient) {
